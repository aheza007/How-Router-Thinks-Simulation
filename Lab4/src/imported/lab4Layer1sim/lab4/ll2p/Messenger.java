...
import imported.lab4Layer1sim.lab4.ll2p.ForwardingTable;
import imported.lab4Layer1sim.lab4.ll2p.LL3PDaemon;
import imported.lab4Layer1sim.lab4.ll2p.RouteTableEntry;
...
public class Messenger implements Runnable {
	...
	public void getObjectReferences(Factory factory){
		...
		FIB = factory.getLRPDaemonShellReference().getFIB(); // forwarding information base, used to populate the active names list.
		layer3Daemon = factory.getLL3PDaemonObjectReference(); // layer 3 daemon, used to send messages.
	...
	private void resetSpinner(){
		...
		for (RouteTableEntry entry : validNetworkList){
			Integer netNumber=entry.getNetworkDistancePair().getNetworkNumber();
			//if (entry.getNetwork().compareTo(-1) > 0 && entry.getNetwork().compareTo(16) < 0)  
				if (netNumber.compareTo(-1) >0 &&netNumber.compareTo(16) < 0)
					spinnerAdapter.add(allNamesAndNetworks.get(netNumber-1)); // network numbers go from 1-15, array elements from 0-14
		}
